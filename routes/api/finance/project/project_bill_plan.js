...
const ProjectBillPlanHandler = require( '@handlers/finance/project/project_bill_plan' );
...
router.use( '/', ( req, res, next ) => {
	...
	req.ProjectBillPlanHandler = new ProjectBillPlanHandler( channelId, userData );
	next();
} );
...
router.get(
	...
	async ( req, res, next ) => {
		try {
			const result = await req.ProjectBillPlanHandler.handleGetAll( req.query );
...
router.post( '/create', validateRoleRequest( [ 'PM' ] ), async ( req, res, next ) => {
	try {
		...
		const result = await req.ProjectBillPlanHandler.handleCreate( req.body );
...
router.put( '/update/:id', validateRoleRequest( [ 'PM' ] ), async ( req, res, next ) => {
	try {
		...
		const result = await req.ProjectBillPlanHandler.handleUpdate( +req.params.id, req.body );
...
router.delete( '/delete/:id', validateRoleRequest( [ 'PM' ] ), async ( req, res, next ) => {
	try {
		const result = await req.ProjectBillPlanHandler.handleDelete( +req.params.id );
